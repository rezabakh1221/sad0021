                    ...
                    if login(username.text,password.text,driver)==1:
                        if get_information(driver,username.text,password.text,ca)==1:
                            if is_raced(driver)==-1:
                                ...
                                await c.edit_message_text(piame.chat.id,ca.message.chat.id,f"📡درحال اتصال به پرتال **¦ {namer} ¦**\n✅دریافت برنامه کلاسی\n",parse_mode="markdown")
                                darss(driver,ca.message.chat.id)
                                await c.edit_message_text(piame.chat.id,ca.message.chat.id,f"📡درحال اتصال به پرتال **¦ {namer} ¦**\n✅دریافت برنامه کلاسی\n✅دریافت لیست درس ها\n",parse_mode="markdown")
                                emtehanat(driver,ca.message.chat.id)
                                await c.edit_message_text(piame.chat.id,ca.message.chat.id,f"📡درحال اتصال به پرتال **¦ {namer} ¦**\n✅دریافت برنامه کلاسی\n✅دریافت لیست درس ها\n✅دریافت برنامه امتحانی\n",parse_mode="markdown")
                                number_do(ca.message.chat.id,c,ca)
                                await c.edit_message_text(piame.chat.id,ca.message.chat.id,f"📡درحال اتصال به پرتال **¦ {namer} ¦**\n✅دریافت برنامه کلاسی\n✅دریافت لیست درس ها\n✅دریافت برنامه امتحانی\n✅دریافت نمرات ترم",parse_mode="markdown")
